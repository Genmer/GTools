...
import manifest from '../../../../src/plugins/pdf-tools/manifest'
...
describe('pdf-tools manifest', () => {
  ...
  it('id 与目录名一致，entry 约定正确，trigger 型无 backend 合法', () => {
    expect(manifest.id).toBe('pdf-tools')
  ...
  it('声明的权限与功能一一对应（无冗余权限）', () => {
    expect(new Set(manifest.permissions)).toEqual(new Set(['dialog', 'fs']))
  })
})
